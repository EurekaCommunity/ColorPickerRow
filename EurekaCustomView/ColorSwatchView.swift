...
class ColorSwatchView : UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        if isSelected {
            let frameColor = color?.blackOrGrayContrastingColor() ?? UIColor.black
            let selectRect = bounds.insetBy(dx: 2.0, dy: 2.0)
            let selectPath = isCircular ? UIBezierPath(ovalIn: selectRect) : UIBezierPath(roundedRect: swatchRect, cornerRadius: CGFloat(Int(swatchRect.width * 0.2)))
            ...
            if frameColor == UIColor.white && false {
                let outlinePath = isCircular ? UIBezierPath(ovalIn: bounds) : UIBezierPath(roundedRect: bounds, cornerRadius: CGFloat(Int(swatchRect.width * 0.2)))
